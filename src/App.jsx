...
// NUEVO MANEJADOR DE PAGO EXITOSO CON MERCADO PAGO BRICKS
  const registrarPagoExitoso = async (idTransaccionMP) => {
    try {
      ...
      setModalPago(null); // Cerramos el modal al terminar de guardar
  ...
  const handleLogin = async (e) => {
    ...
    if (error) {
      ...
      if (usuarioEncontrado.estado_verificacion === 'rechazado') {
        ...
        enviarNotificacion(usuarioEncontrado.email, "Solicitud Rechazada", "Tu documentación para ser propietario no fue aprobada. Continuarás usando la plataforma con tu rol de Inquilino.");
        ...
        enviarNotificacion(usuarioEncontrado.email, "¡Felicidades, eres Propietario!", "Tu solicitud fue verificada con éxito. Ya puedes empezar a publicar tus inmuebles.");
...
// GESTIÓN DE USUARIOS (Dar de baja / Reactivar con EFECTO CASCADA)
    const handleToggleEstadoUsuario = async (id, estadoActual, emailUsuario) => {
      ...
      if(window.confirm(`¿Estás seguro de que deseas cambiar el estado de este usuario a ${nuevoEstado}?`)) {
        ...
        if (nuevoEstado === 'inactivo') {
          enviarNotificacion(emailUsuario, "Cuenta inhabilitada", "Tu cuenta ha sido suspendida por un administrador. Por seguridad, tus publicaciones también fueron dadas de baja.");
        } else {
          enviarNotificacion(emailUsuario, "Cuenta reactivada", "Tu cuenta ha sido reactivada. Ya puedes volver a iniciar sesión en Zenith Alquileres.");
        }
      }
    };
      ...
      if(window.confirm(mensajeConfirmacion)) {
        ...
        if (error) {
          ...
          if (emailReal && nuevoEstado === 'inactivo') {
            enviarNotificacion(emailReal, "Publicación dada de baja", `Tu inmueble "${prop.titulo}" ha sido dado de baja por un administrador por incumplir las normativas de la plataforma.`);
          } else if (emailReal && nuevoEstado === 'activo') {
            enviarNotificacion(emailReal, "Publicación reactivada", `Tu inmueble "${prop.titulo}" ha sido reactivado y vuelve a estar visible en el buscador.`);
          }
        }
      }
    };
...
// Simulador de Notificaciones por Email (RF18)
  const enviarNotificacion = (emailDestino, asunto, mensaje) => {
    console.log(`[EMAIL SIMULADO A: ${emailDestino}] Asunto: ${asunto} | Cuerpo: ${mensaje}`);
    const nuevaNotificacion = { id: crypto.randomUUID(), email: emailDestino, texto: `${asunto}: ${mensaje}`, leida: false, fecha: new Date() };
...
// NUEVO: Abrir notificaciones y marcarlas todas como leídas
  const handleAbrirNotificaciones = () => {
    ...
    // 2. Actualizamos el estado para que todas las de este usuario pasen a leida: true
    setNotificaciones(notificaciones.map(noti => 
      noti.email === usuarioLogueado.email ? { ...noti, leida: true } : noti
    ));
  };
 ...
 // Iniciar un nuevo chat desde una propiedad (RF17)
  const handleConsultar = async (propiedadId) => {
    ...
    if (mensajeText && mensajeText.trim() !== '') {
      ...
      // Si tienes la función de notificaciones activada, la dejamos aquí:
      // enviarNotificacion(emailRealDueño, "Nueva consulta recibida", `${usuarioLogueado.nombre} te ha escrito sobre tu propiedad.`);
  ...
  const handleEnviarMensaje = async (e) => {
    ...
    const destinatario = usuarioLogueado.email === chatActivo.propietarioEmail ? chatActivo.inquilinoEmail : chatActivo.propietarioEmail;
    enviarNotificacion(destinatario, "Nuevo mensaje", `Tienes una nueva respuesta de ${usuarioLogueado.nombre}.`);
  };
...
const handleEnviarProblema = async (e) => {
    ...
    if (emailRealDueño) {
      enviarNotificacion(emailRealDueño, "Problema Reportado", `Tu inquilino ha reportado un problema: "${descripcion}"`);
    }
      ...
      <div className="landing-container">
        ...
        <nav className="landing-navbar">
          <div className="landing-logo">
            <strong>Zenit</strong> <span style={{ color: '#3498db' }}>Alquileres</span>
          </div>
          <div className="landing-nav-links">
           <button className="btn-nav-simple" onClick={() => setVistaActual('buscador')}>
              Propiedades
            </button>
            <button className="btn-nav-primary" onClick={() => setVistaActual('login')}>
              {usuarioLogueado ? 'Ir a mi Panel' : 'Iniciar Sesión'}
            </button>
          </div>
        </nav>
        ...
        <header className="landing-hero">
          ...
          <div className="hero-content">
            <h1>¿Casas? Tenemos.<br/>¿Excusas? No.</h1>
          </div>
        </header>
        ...
        <section className="landing-servicios">
          ...
          <div className="servicios-grid">
            ...
            <div className="servicio-card">
              ...
              <p>¿Estás pensando en alquilar tu casa?</p>
              <button className="btn-servicio" onClick={() => {
            ...
            <div className="servicio-card">
              <h3>BUSCÁ TU NUEVO HOGAR</h3>
              <p>¡Queremos ayudarte!</p>
              <button className="btn-servicio" onClick={() => setVistaActual('buscador')}>
                Buscar
              </button>
            </div>
        ...
        <section className="landing-info">
          ...
          <div className="info-row">
            <div className="info-img-container">
              {/* Nueva foto de una casa hermosa */}
              <img src="https://images.unsplash.com/photo-1600596542815-ffad4c1539a9?ixlib=rb-4.0.3&auto=format&fit=crop&w=800&q=80" alt="Departamento moderno" />
            </div>
            <div className="info-text">
              <h2>+200 propiedades activas</h2>
              <p>Descubre un catálogo único con más de 200 propiedades activas listas para convertirse en tu próximo hogar o inversión. Desde modernos departamentos en el centro de la ciudad hasta amplias casas familiares en zonas tranquilas, pasando por terrenos con gran potencial, tenemos opciones para todos los gustos y presupuestos. Cada propiedad ha sido cuidadosamente seleccionada para ofrecerte calidad, ubicación estratégica y comodidad. ¡La llave de tu nuevo comienzo puede estar a un clic de distancia!</p>
            </div>
          </div>

          {/* Fila 2: Texto izquierda, Imagen derecha (SIN la clase reversa) */}
          <div className="info-row">
            <div className="info-text">
              <h2>120 contratos cerrados en el último mes</h2>
              <p>Nuestra experiencia y dedicación se reflejan en los resultados: en el último mes, ayudamos a más de 120 personas a encontrar su hogar ideal o cerrar acuerdos de alquiler exitosos. Este logro demuestra nuestro compromiso con la excelencia y la satisfacción de nuestros clientes.</p>
            </div>
            <div className="info-img-container">
              {/* Nueva foto de firma de contrato/llaves */}
              <img src="https://images.unsplash.com/photo-1556912172-45b7abe8b7e1?ixlib=rb-4.0.3&auto=format&fit=crop&w=800&q=80" alt="Contrato exitoso" />
            </div>
          </div>
    ...
    return (
      ...
      <div className="auth-container">
        ...
        <div className="perfil-nav-top">
          <button type="button" className="btn-volver-limpio" onClick={() => setVistaActual('buscador')}>
            ⬅ Volver al Inicio
          </button>
        </div>
  ...
  if (vistaActual === 'buscador') {
    ...
    return (
      <div className="buscador-container">
       {/* ========================================================
            ENCABEZADO GLOBAL MODERNO (INTELIGENTE)
            ======================================================== */}
        <header className="header-dashboard-moderno">
          <div className="header-logo">
            <h2>Zenith Alquileres</h2>
          </div>
           ...
           {!usuarioLogueado ? (
                  <>
                    ...
                    <button
                      ...
                      style={{ margin: 0, padding: '8px 16px', backgroundColor: '#3498db', width: 'auto' }}
              ...
              /* Si ESTÁ LOGUEADO (Inquilino, Propietario o Admin) */
              <>
                <span className="header-greeting">👋 Hola, {usuarioLogueado.nombre}</span>

                <button 
                  className={`nav-btn ${vistaActual === 'buscador' ? 'nav-btn-active' : ''}`} 
                ...
                {(usuarioLogueado.rol === 'propietario' || usuarioLogueado.rol === 'admin') && (
                  <button 
                    className={`nav-btn ${['mi-panel', 'mensajes', 'panel-cobros'].includes(vistaActual) ? 'nav-btn-active' : ''}`} 
                    onClick={() => setVistaActual(usuarioLogueado.rol === 'admin' ? 'panel-admin' : 'mi-panel')}
                  >
                    ⚙️ {usuarioLogueado.rol === 'admin' ? 'Panel Admin' : 'Mi Panel'}
                  </button>
                )}
                ...
                {usuarioLogueado.rol !== 'admin' && (
                  <>
                    <button 
                      className={`nav-btn ${vistaActual === 'mis-alquileres' ? 'nav-btn-active' : ''}`} 
                      onClick={() => setVistaActual('mis-alquileres')}
                    >
                      🏠 Mis Alquileres
                    </button>
                    ...
                    {usuarioLogueado.rol !== 'propietario' && (
                      <button 
                        className={`nav-btn ${vistaActual === 'mensajes' ? 'nav-btn-active' : ''}`} 
                    ...
                    {(() => {
                      const noLeidas = notificaciones.filter(n => n.email === usuarioLogueado.email && !n.leida).length;
                      return (
                        <button className="nav-btn btn-notificaciones" onClick={handleAbrirNotificaciones}>
                          🔔 Notif.
                          {noLeidas > 0 && <span className="badge-notificacion">{noLeidas}</span>}
                        </button>
                      );
                    })()}
                  </>
                ...
                <button className="nav-btn" onClick={() => setVistaActual('perfil')}>
                  👤 Perfil
                </button>
                  {usuarioLogueado?.rol === 'inquilino' && (
                      <button
                        className={`nav-btn ${vistaActual === 'registro-propietario' ? 'nav-btn-active' : ''}`}
                        onClick={() => setVistaActual('registro-propietario')}
                      >
                        🏷️ ¿Querés publicar?
                      </button>
                    )}
                <button className="nav-btn btn-salir" onClick={() => { setUsuarioLogueado(null); setVistaActual('inicio'); }}>
                  🚪 Salir
                </button>
              </>
            )}
        ...
        <div className="buscador-layout">
          ...
          <aside className="filter-panel">
            ...
            <div className="filter-group">
              ...
              {filtros.fechaInicio && filtros.fechaFin && filtros.fechaInicio > filtros.fechaFin && (
                <span className="filtro-fecha-error">La salida es anterior al ingreso.</span>
              )}
            </div>

            <div className="filter-group">
              <label className="filter-label">Tipo de Operación</label>
              <select className="filter-input" name="tipoOperacion" value={filtros.tipoOperacion} onChange={handleFiltroChange}>
            ...
            <div className="filter-group">
              ...
              <select className="filter-input" name="tipoInmueble" value={filtros.tipoInmueble} onChange={handleFiltroChange}>
                <option value="">Todos</option>
            ...
            {filtros.tipoOperacion !== 'temporal' && (
              <div className="filter-group">
                <label className="filter-label">Tipo de Garantía</label>
                <select className="filter-input" name="tipoGarantia" value={filtros.tipoGarantia} onChange={handleFiltroChange}>
            ...
            <div className="filter-group">
              <label className="filter-label">Comodidades</label>
              <div className="checkbox-list">
                <label className="checkbox-item"><input type="checkbox" name="mascotas" checked={filtros.mascotas} onChange={handleFiltroChange} /> Mascotas permitidas</label>
                <label className="checkbox-item"><input type="checkbox" name="cochera" checked={filtros.cochera} onChange={handleFiltroChange} /> Cochera</label>
                <label className="checkbox-item"><input type="checkbox" name="balcon" checked={filtros.balcon} onChange={handleFiltroChange} /> Balcón</label>
                <label className="checkbox-item"><input type="checkbox" name="internet" checked={filtros.internet} onChange={handleFiltroChange} /> Internet</label>
                <label className="checkbox-item"><input type="checkbox" name="gas" checked={filtros.gas} onChange={handleFiltroChange} /> Gas</label>
                <label className="checkbox-item"><input type="checkbox" name="luz" checked={filtros.luz} onChange={handleFiltroChange} /> Electricidad</label>
                <label className="checkbox-item"><input type="checkbox" name="agua" checked={filtros.agua} onChange={handleFiltroChange} /> Agua</label>
              </div>
            </div>
          </aside>
          ...
          {/* =========================================
              ...
              ========================================= */}
          <main className="feed-propiedades">
            ...
            {propiedadesFiltradas.length === 0 ? (
              ...
              <div className="propiedades-grid">
                {propiedadesFiltradas.map(prop => {
                  ...
                  return (
                    <div key={prop.id} className="card-inmueble">
                      <div className="card-inmueble-img-wrapper">
                        <img src={fotoPrincipal} alt={prop.titulo} className="card-inmueble-img" />
                      </div>
                      
                      <div className="card-inmueble-body">
                        <h3 className="card-inmueble-titulo" title={prop.titulo}>{prop.titulo}</h3>
                        
                        <p className="card-inmueble-ubicacion">
                          {prop.direccion}, {prop.ciudad}
                        </p>

                        <div className="card-badges-row">
                          <span className={`badge-moderno ${tipoOperacion === 'temporal' ? 'badge-temporal' : 'badge-largo-plazo'}`}>
                            {tipoOperacion === 'temporal' ? 'Temporal' : 'Largo Plazo'}
                          </span>
                          <span className="badge-moderno badge-tipo" style={{ textTransform: 'capitalize' }}>
                            {tipoInmueble}
                          </span>
                        </div>

                        <div className="card-inmueble-precio-box">
                          <span className="card-precio-valor">
                            ${tipoOperacion === 'largo_plazo' ? alquilerMensual : pagoDiario}
                          </span>
                          <span className="card-precio-sufijo">
                            {tipoOperacion === 'largo_plazo' ? ' / mes' : ' / día'}
                          </span>
                        </div>
...
// VISTA 11: MIS ALQUILERES Y PAGOS (RF19 al RF24)
  if (vistaActual === 'mis-alquileres' && usuarioLogueado) {
    ...
    return (
      <div className="buscador-container dashboard-panel-bg">
        ...
        <header className="header-dashboard-moderno">
          ...
          <div className="header-nav-links">
            ...
            <button className="nav-btn" onClick={() => setVistaActual('perfil')}>👤 Perfil</button>
            ...
            <button className="nav-btn btn-salir" onClick={() => { setUsuarioLogueado(null); setVistaActual('inicio'); }}>🚪 Salir</button>
          </div>
        </header>
      ...
      {/* ========================================================
        ...
        <header className="header-dashboard-moderno">
          ...
          <div className="header-nav-links">
            ...
            {!usuarioLogueado ? (
              <>
                <button className="nav-btn" onClick={() => setVistaActual('login')} style={{ fontWeight: 'bold', color: '#3498db' }}>
                  Iniciar Sesión
                </button>
                <button className="btn-submit" style={{ margin: 0, padding: '8px 16px', backgroundColor: '#3498db', width: 'auto' }} onClick={() => setVistaActual('registro')}>
                  Registrarse
                </button>
              </>
            ) : (
              ...
              <>
                ...
                {/* Todos los logueados ven "Explorar Propiedades" */}
                <button 
                  className={`nav-btn ${vistaActual === 'buscador' ? 'nav-btn-active' : ''}`} 
                  onClick={() => setVistaActual('buscador')}
                >
                ...
                {/* MENÚ EXCLUSIVO PROPIETARIOS Y ADMIN */}
                {(usuarioLogueado.rol === 'propietario' || usuarioLogueado.rol === 'admin') && (
                  <button 
                    className={`nav-btn ${['mi-panel', 'mensajes', 'panel-cobros'].includes(vistaActual) ? 'nav-btn-active' : ''}`} 
                    onClick={() => setVistaActual(usuarioLogueado.rol === 'admin' ? 'panel-admin' : 'mi-panel')}
                ...
                {/* MENÚ PARA INQUILINOS Y PROPIETARIOS (El Admin no necesita ver esto) */}
                {usuarioLogueado.rol !== 'admin' && (
                  <>
                    <button 
                      className={`nav-btn ${vistaActual === 'mis-alquileres' ? 'nav-btn-active' : ''}`} 
                    ...
                    {/* Botón de Mensajes directo para Inquilinos */}
                    {usuarioLogueado.rol !== 'propietario' && (
                      <button 
                        className={`nav-btn ${vistaActual === 'mensajes' ? 'nav-btn-active' : ''}`} 
                        onClick={() => setVistaActual('mensajes')}
  ...
  if (vistaActual === 'mensajes' && usuarioLogueado) {
    ...
    return (
      <div className="buscador-container dashboard-panel-bg">
        ...
        {/* ========================================================
            1. UN SOLO ENCABEZADO GLOBAL INTELIGENTE PARA TODOS
            ======================================================== */}
        <header className="header-dashboard-moderno">
          <div className="header-logo">
            <h2>Zenith Alquileres</h2>
            ...
            <div className="chat-main" style={usuarioLogueado.rol === 'propietario' ? { backgroundColor: '#fbfcfc' } : {}}>
              {chatActivo ? (
                <>
                  ...
                  <form onSubmit={handleEnviarMensaje} className="chat-input-area">
      ...
      {/* ========================================================
        ...
        <div style={{ maxWidth: '1000px', margin: '40px auto', padding: '0 20px' }}>
          ...
          <button 
              ...
              style={{ backgroundColor: '#3498db', margin: 0, width: 'auto', padding: '12px 24px', borderRadius: '8px', fontWeight: 'bold', border: 'none', cursor: 'pointer', color: 'white', boxShadow: '0 4px 6px rgba(52, 152, 219, 0.2)' }}
            >
              + Publicar Inmueble
            </button>
          </div>
        ...
        <div className="auth-card" style={{ maxWidth: '800px' }}>
          ...
          <form onSubmit={handlePublicarInmueble} className="auth-form">
            ...
            <div className="form-acciones-flex">
              <button type="submit" className="btn-submit" style={{ backgroundColor: '#8e44ad', margin: 0, flex: 1 }}>
...
/// VISTA 14: BANDEJA DE NOTIFICACIONES
  if (vistaActual === 'notificaciones' && usuarioLogueado) {
    const misNotis = notificaciones.filter(n => n.email === usuarioLogueado.email).reverse(); 
    
    return (
      <div className="buscador-container vista-espaciada">
        {/* ========================================================
            ENCABEZADO GLOBAL MODERNO (INTELIGENTE)
            ======================================================== */}
        <header className="header-dashboard-moderno">
            ...
            <div className="notificaciones-lista">
              {misNotis.map(noti => (
                <div key={noti.id} className="notificacion-item">
                  <p className="notificacion-texto">{noti.texto}</p>
      ...
      <div className="auth-container">
        <div className="auth-card">
         ...
         <div className="toggle-view" style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
              <button className="btn-link" type="button" onClick={() => { setErrores({}); setVistaActual('recuperar-password'); }} style={{ color: '#e74c3c', fontSize: '0.9rem' }}>
   ...
   // Ya vienen filtrados de Supabase, así que solo lo asignamos
    ...
    const procesarSolicitud = async (id, accion, emailUsuario) => {
      ...
      if (error) {
        ...
        enviarNotificacion(emailUsuario, "Actualización de cuenta", `Tu solicitud para ser propietario ha sido ${accion}.`);
    ...
    const procesarPropiedad = async (prop, accion, motivo = "") => {
      ...
      if (error) {
        ...
        if (emailReal) {
          ...
          enviarNotificacion(emailReal, "Revisión de Publicación", mensajeNotificacion);
        }
      }
    };
